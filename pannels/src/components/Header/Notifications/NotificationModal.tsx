...
const NotificationModal: React.FC<IProps> = ({ notify }) => {
  ...
  const sendData = async (choice: IChoice) => {
    ...
    const res = await axios.patch(`${baseAdminUrl}/companyNotify/${notify.id}/`, choice)
    return res.data
  }
  ...
  const subject = notify.status === 'create' ? `درخواست ثبت شرکت ${notify.name}` : `درخواست ویرایش اطلاعات شرکت ${notify.name}`
